...
jQuery(window).load(function() {
	jQuery(window).trigger('imageresize');
});

// searchbox autocomplete
function initSearchForms() {
	// simple autocomplete demo
	var parentForm = jQuery('.navbar-form');
	var autocompleteDrop = jQuery('.autocomplete');
	var globalParent = jQuery('html');
	var clickedInForm;
	
	// click event handlers
	parentForm.on('click', function() {
		parentForm.addClass('focus');
	});
	
	globalParent.on('click',function(e){
		clickedInForm = jQuery(e.target).closest(parentForm).length > 0;
		
		if(!clickedInForm){
			parentForm.removeClass('focus');
			autocompleteDrop.hide()
		}
	});
	
	jQuery('.navbar-form').autoCompleteForm({
		resultsHolder: 'div.autocomplete',
...
function initAddClasses() {
	jQuery('.login-holder .change-veiw').clickClass({
		classAdd: 'form-active',
		addToParent: 'login-holder'
	});
}
...
;(function($,window){
	...
	$.fn.autoCompleteForm = function(opt) {
		...
		return this.each(function(){
			var form = $(this);
			var target = form.attr('data-autocomplete');
			...
			function selectItem(obj, realEvent) {
				...
				if(opt.listItemsFillsInput) {
					input.val(jQuery.trim(obj.text())).focus();
					return false;
				} else {
					// example redirect
					if(!realEvent) {
			...
			jQuery(opt.deleteButton).on('click', function(e) {
				e.preventDefault();
				input.val('');
			});
...
var ImageStretcher = {
	...
	add: function(options) {
		...
		if(!this.win) {
			...
			this.win.on('resize orientationchange imageresize', this.resizeHandler);
		}
